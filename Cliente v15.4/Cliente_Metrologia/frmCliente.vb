...
Public Class frmCliente
    ...
    Dim Informes As New Negocio_HInformes()
    Private Declare Auto Function SetProcessWorkingSetSize Lib "kernel32.dll" (ByVal procHandle As IntPtr, ByVal min As Int32, ByVal max As Int32) As Boolean


    '**********************************************para firmar el documento***************************************************************
    '1. generamos el qr
    Public Sub Generar_Qr(Codigo As String, lugar_cal As String)
        'SI EL ARCHIVO EXISTE NO SEGENERA EL CODIGO QR
        If File.Exists("C:\archivos_metrologia\Imagenes_QR\" & Codigo & ".jpg") Then
            'SI LA IMG NO EXISTE SE GENERA EL QR
        Else

            'Ac� tomamos el primer certificado de mi repositorio personal
            'En producci�n deber�a elegir el certificado a utilizar
            Dim objStore As X509Store = New X509Store(StoreName.My, StoreLocation.CurrentUser)
            objStore.Open(OpenFlags.ReadOnly)
            Dim objCert As X509Certificate2 = Nothing
            If (Not (objStore.Certificates) Is Nothing) Then
                For Each objCertTemp As X509Certificate2 In objStore.Certificates
                    If objCertTemp.HasPrivateKey Then
                        objCert = objCertTemp
                        Exit For
                    End If
            ...
            Else
                ...
                QrCodeImgControl1.Text = "FIRMADO POR: " & DF(0).Replace("CN=", "").ToString() & "" & vbCrLf & "CERTIFICADO:" & Codigo & "" & vbCrLf & "LOCALIZACION:" & lugar_cal & "" & vbCrLf & "FECHA: " & Fecha_actual & "" & vbCrLf & "VALIDAR CON:" & vbCrLf & "www.firmadigital.gob.ec 2.7.0"
                Dim Img As Image = DirectCast(QrCodeImgControl1.Image.Clone, Image)
                Img.Save("C:\archivos_metrologia\Imagenes_QR\" & Codigo & ".jpg")
        ...
        End If
    End Sub
    '2.Insertamos la imagen en el word prevamente 


    Private Sub Firmar_Documento(Documento_Original As String, Documento_Firmado As String, C_Proyecto As String)
        Try
            ...
            If (objCert Is Nothing) Then
                MessageBox.Show("No posee certificados personal con clave privada")
            Else
                PDF.SignHashed(Documento_Original, Documento_Firmado, objCert, C_Proyecto, "", True)
    ...
    Private Sub Timer1_Elapsed(sender As Object, e As Timers.ElapsedEventArgs) Handles Timer1.Elapsed
        Try
            Dim minutos As String = DateTime.Now.ToString("hh:mm")
            'Try
            '    Label2.Text = Date.Now.ToLongTimeString
            '    minutos = Mid(Label2.Text, 4)
            'Catch ex As Exception
            '    'Application.Restart()
        ...
        Try
            ...
            For i = 0 To 9
                ...
                If nombre <> Nothing Then
                    ...
                    eliminarFichero(dir_completa, usu_ftp, pas_ftp)

                End If
            Next
            reader.Close()
            'eliminacion del fechiro 
    ...
    Function Verificar(codigo_recibido As String, Instruccion As String) As String
        ...
        Try
            ...
            codigo_recibido = codigo_recibido.Replace("'", "")
    ...
    End Function
    '************************Ingreso de la fecha de recpcion de los informes************


    Private Sub InsertarFechas(codigo_recibido As String)
        '   Dim Respuesta As String
        Try
            ccn.Open()
            Dim consultas As String
            codigo_recibido = codigo_recibido.Replace("'", "")
            consultas = "update Balxpro set FechaRecepcion=getdate() where IdeComBpr = '" & codigo_recibido & "'"
            Dim comando = New SqlCommand(consultas, ccn)
            Dim result = Convert.ToString(comando.ExecuteNonQuery())
            ccn.Close()
    ...
    Sub Leer()
        Try
            ...
            While lector.Peek() <> -1
                ...
                ' obtenemos los datos del archivo que genera la tablet
                linea = linea
                Contenedor += linea
                ' si el metrologo crea una nueva balanza desde la tablet obtenemos de referencia IF NOT EXISTS
                Dim Inicio_balanza As Integer = InStr(linea, "IF NOT EXISTS")
                If Inicio_balanza <> 0 Then
                    If linea.Contains("END;") Then
                        Inicio_balanza = 0

                        Try
                            ccn.Open()
                            Dim ObjWriter = New SqlDataAdapter()
                            ObjWriter.InsertCommand = New SqlCommand(Contenedor, ccn)
                    ...
                    End If
                Else
                    'insertamos los datos de las pedientes  balanzas 
                    If linea.Contains("End") Then
                        pos = InStr(Contenedor, "IdeComBpr")
                        Codigo_Proyecto = (Mid(Contenedor, pos + 12, 8))
                        If Verificar(Codigo_Proyecto, "").Equals("si") Then
                            ...
                            Catch ex As Exception
                                ...
                                ccn.Close()
                            End Try
                            InsertarFechas(Codigo_Proyecto)
                            Contenedor = ""
                        Else
    ...
    Public Function conectar() As Boolean
        ...
        Try
            ...
            If servidor <> "" And base <> "" And usuario <> "" And password <> "" Then
                'revertir el cambio despues de la actualizacion
                ...
                'ccn.ConnectionString = "Data Source=SISTEMAS\SQL2014;Initial Catalog=SisMetPrec;User ID=sa;Password=Sistemas123;MultipleActiveResultSets=True"
            End If
...
aqui:
    ...
    Private Sub imprimir()
        ...
        Dim unidad_recibida As String = ""
        Dim proyec As String = ""
        Dim lugar_Calibracion As String = ""

        Try
            If (ccn.State = ConnectionState.Open) Then
                ccn.Close()
            End If
            ccn.Open()

            Dim datos As New DataSet
            datos = Informes.Informes_Registrador()

            For Each row As DataRow In datos.Tables(0).Rows
                'row(0).ToString()
                lector_0 = row(0).ToString()
                lector_1 = row(1).ToString()
                lector_2 = row(2).ToString()
                unidad_recibida = row(3).ToString()
                proyec = row(4).ToString()
                ...
                Select Case lector_1
                    ...
                    Case "Camionera"
                        ...
                        'matar_word()
                        Impresa_Cam(lector_0, lector_2, "O")
                End Select
    ...
    Private Sub imprimir_d()
        ...
        Try
            ccn.Open()
            Dim StrSQL As String = "SELECT CodBpr,ClaBpr,ideBpr FROM Balxpro WHERE est_esc = 'DS' and estado !='Terminado' "
            Dim ObjCmd As SqlCommand = New SqlCommand(StrSQL, ccn)
            Dim ObjReader = ObjCmd.ExecuteReader
            While (ObjReader.Read())
                lector_0 = (ObjReader(0).ToString())
        ...
        End Try
    End Sub
    Private Sub Impresa_Cam(codigobpr As String, idebpr As String, tipo As String)
        ...
        Try
            ...
            If tipo.Equals("P") Then
                ...
                oDoc = oWord.Documents.Add("C:\archivos_metrologia\Plantillas\documentosQR\Camionera_Preliminar.dotx")
            Else
                oDoc = oWord.Documents.Add("C:\archivos_metrologia\Plantillas\documentosQR\Camionera.dotx")
            ...
            If conteo = 1 Then
                ...
                Dim Str3 As String = "select IdeBpr,fec_cal,desbpr,marbpr,modbpr,serbpr,CapMaxBpr,CapUsoBpr,DivEscBpr,UnidivEscBpr,DivEsc_dBpr,UbiBpr,codpro,recporclibpr,identbpr,ranbpr,lugcalBpr,CONVERT(DATE,FechaLiberacion,103),(select MAX(CarPca) from PCarga_Cab where IdeComBpr='" & IdeComBpr & "')  " &
                                             "from Balxpro " &
                ...
                While (ObjReader3.Read())
                    ...
                    hoy = ObjReader3(17).ToString
                    calibrada = ObjReader3(18).ToString
                End While
                ObjReader3.Close()
            Else
                ...
                Dim Str3 As String = "select IdeComBpr,fec_cal,desbpr,marbpr,modbpr,serbpr,CapMaxBpr,CapUsoBpr,DivEscBpr,UnidivEscBpr,DivEsc_dBpr,UbiBpr,codpro,recporclibpr,identbpr,ranbpr,lugcalBpr,CONVERT(DATE,FechaLiberacion,103),(select MAX(CarPca) from PCarga_Cab where IdeComBpr='" & IdeComBpr & "') " &
                                             "from Balxpro " &
                                             "where codBpr = " & codigobpr & ""
                Dim ObjCmd3 As SqlCommand = New SqlCommand(Str3, ccn)
                Dim ObjReader3 = ObjCmd3.ExecuteReader
            ...
            If tipo.Equals("P") Then
                ...
                ' oDoc = oWord.Documents.Add("C:\archivos_metrologia\Plantillas\ClaseIII_Preliminar.dotx")
            Else
                ...
                For value As Integer = 1 To 2
                    ...
                    Dim start_range As Word.Range = oDoc.Bookmarks.Item("Codigo_qr" & value).Range
                    ' Dim start_range As Word.Range = oDoc.Bookmarks.Item("Codigo_qr2").Range

                    ' Add the picture to the Range's InlineShapes.
                    Dim picture_file As String = "C:\archivos_metrologia\Imagenes_QR\" & IdeComBpr & ".jpg"
                ...
                Microsoft.Office.Core.MsoTriState.msoTrue
                    ...
                    shape.Left = CSng(Word.WdShapePosition.wdShapeCenter)
            ...
            hoy = UCase(Format(Convert.ToDateTime(hoy), "yyyy-MM-dd"))
            Dim fec_Calibracion As String = UCase(Format(Convert.ToDateTime(feccal), "yyyy-MM-dd"))
            ...
            oDoc.Bookmarks.Item("calibrada").Range.Text = Replace(calibrada, ",", "")
            ...
            oDoc.Bookmarks.Item("feccalibracion2").Range.Text = feccal
            ...
            Str1 = "select CodPca_c,CarPca,NumPca from PCarga_Cab " &
                                     "where IdeComBpr = '" & IdeComBpr & "' order by NumPca"
            ObjCmd1 = New SqlCommand(Str1, ccn)
            ObjReader1 = ObjCmd1.ExecuteReader
            While (ObjReader1.Read())
            ...
            Str1 = "SELECT replace(fec_proxBpr,'-','/') " &
            ...
            If tipo.Equals("P") Then
                nombre_carp = carpeta & "\" & nombre_arch & "- Preliminar.docx" 'nombre completo "path"
                nombre_pdf = carpeta & "\" & nombre_arch & "- Preliminar.pdf" 'nomre completo "path" para el formato .pdf
            Else
                nombre_carp = carpeta & "\" & nombre_arch & ".docx" 'nombre completo "path"



                ''************************Codigo QR DEL DOCUMENTO  DOCUMENTOS ***********************************************
                'Dim qr_imagen As New Generar_QR()
                'Dim ap As New Microsoft.Office.Interop.Word.Application()
                'Dim documento As Microsoft.Office.Interop.Word.Document = ap.Documents.Open(nombre_carp)
                'qr_imagen.reemplazarImagen(documento, "Codigo_qr", "C:\archivos_metrologia\Imagenes_QR\" & IdeComBpr & ".jpg")

                ''****************

                nombre_pdf = carpeta & "\" & nombre_arch & ".pdf" 'nomre completo "path" para el formato .pdf
            ...
            Else
                ''*************************Firma del Documento********************************
                Firmar_Documento(nombre_pdf, carpeta & "\" & nombre_arch & "-firma.pdf", IdeComBpr)
                ''*******************************************************************************************
            End If
    ...
    End Sub
    Private Sub Impresa_II(codigobpr As String, idebpr As String, tipo As String)
        ...
        Try
            ...
            ' oWord.Visible = True
            oWord.Visible = False
            oDoc = oWord.Documents.Add("C:\archivos_metrologia\Plantillas\documentosQR\ClaseII.dotx")
            '///***
            Dim conteo As Integer = 0
            Dim Str2 As String = "select count(LitBpr) from Balxpro where idebpr=" & idebpr & ""
            Dim ObjCmd2 As SqlCommand = New SqlCommand(Str2, ccn)
            ...
            Dim Str_j As String = "SELECT dbo.Clientes.NomCli,Idebpr,IdeComBpr " &
                                                "From dbo.Balxpro INNER Join dbo.Proyectos ON dbo.Balxpro.CodPro = dbo.Proyectos.CodPro " &
                                                "INNER Join dbo.Clientes ON dbo.Proyectos.CodCli = dbo.Clientes.CodCli " &
                                                "Where (dbo.Balxpro.CodBpr = '" & codigobpr & "')"

            Dim ObjCmd_j As SqlCommand = New SqlCommand(Str_j, ccn)
            Dim ObjReader_j = ObjCmd_j.ExecuteReader
            While (ObjReader_j.Read())
                nombrecli = ObjReader_j(0).ToString()
                ide = ObjReader_j(1).ToString()
                IdeComBpr = ObjReader_j(2).ToString()
            End While
            ObjReader_j.Close()
            ide_anio = Mid(ide, 1, 2)
            ide_mes = Mid(ide, 3, 2)
            ...
            Else
                ...
                While (ObjReader3.Read())
                    If (IdeComBpr.Length = 8) Then
                        nombre = Mid(ObjReader3(0).ToString(), 1, 6) & "-" & Mid(ObjReader3(0).ToString(), 7, 2)
            ...
            While (ObjReader6.Read())
                ...
                Certs = Certs & certif & ", "
                oTable.Cell(fila, 1).Range.Text = certif
                Dim Str7 As String = "select distinct(FecCer),TipCer,IdeCer from Certificados where NomCer ='" & certif & "' "
                Dim ObjCmd7 As SqlCommand = New SqlCommand(Str7, ccn)
                Dim ObjReader7 = ObjCmd7.ExecuteReader
                While (ObjReader7.Read())
                    fec_cert = ObjReader7(0).ToString()
            ...
            End While
            ObjReader1.Close()
            '***********PRUEBA DE CARGA aumento de la fila 2021/01/25
            Dim conta_lineas As Integer = 2
            oTable = oDoc.Tables.Add(oDoc.Bookmarks.Item("pruebacarga").Range, cta_carga + 1, 7)
            oTable.Borders.Enable = 1
            oTable.Cell(1, 1).Range.Text = "N°"
            ...
            While (ObjReader1.Read())
                ...
                Str2 = "select LecAscRes,LecDesRes,ErrAscRes,ErrDesRes from Results  " &
                                     "where IdeComBpr = '" & IdeComBpr & "' AND NUMRES='" & ObjReader1(2).ToString() & "'" '" & (ObjReader1(0).ToString()) & ""
                ObjCmd2 = New SqlCommand(Str2, ccn)
                ObjReader2 = ObjCmd2.ExecuteReader
                While (ObjReader2.Read())
                    oTable.Cell(conta_lineas, 3).Range.Text = Replace(formateo((ObjReader2(0).ToString()), 1), ",", "")
            ...
            Dim conta_incerti As Integer = 2
            oTable = oDoc.Tables.Add(oDoc.Bookmarks.Item("incertidumbre").Range, cta_incerti + 1, 10)
            oTable.Borders.Enable = 1
            oTable.Cell(1, 1).Range.Text = "N°"
            oTable.Cell(1, 2).Range.Text = "CARGA " & unidad
            oTable.Cell(1, 3).Range.Text = "C.CONVENCIONAL" ' & unidad
            oTable.Cell(1, 4).Range.Text = "LEC. ASC " & unidad
            oTable.Cell(1, 5).Range.Text = "ERROR ASC " & unidad
            oTable.Cell(1, 6).Range.Text = "LEC. DESC " & unidad
            oTable.Cell(1, 7).Range.Text = "ERROR DSC " & unidad
            oTable.Cell(1, 8).Range.Text = "k"
            oTable.Cell(1, 9).Range.Text = "U " & unidad
            oTable.Cell(1, 10).Range.Text = "U " & unidad

            oTable.Rows.Item(1).Range.Font.Bold = False
            oTable.Rows.Item(1).Range.Font.Italic = False
            oTable.Rows.HeightRule = Word.WdRowHeightRule.wdRowHeightExactly
            oTable.Rows.Item(1).Height = 10
            Dim datos_MasaConencional As New DataSet
            datos_MasaConencional = Informes.Masa_Conveccioanla(IdeComBpr, conta_incerti)
            If (ccn.State = ConnectionState.Open) Then
                ccn.Close()
            End If
            ccn.Open()
            ...
            While (ObjReader1.Read())
                ...
                'carga convencional en clase II


                oTable.Cell(conta_incerti, 4).Range.Text = Replace(formateo((ObjReader1(2).ToString()), 1), ",", "")
                oTable.Cell(conta_incerti, 5).Range.Text = Replace(formateo((ObjReader1(3).ToString()), 1), ",", "")
                oTable.Cell(conta_incerti, 6).Range.Text = Replace(formateo((ObjReader1(4).ToString()), 1), ",", "")
                oTable.Cell(conta_incerti, 7).Range.Text = Replace(formateo((ObjReader1(5).ToString()), 1), ",", "")
                'oTable.Cell(conta_incerti, 7).Range.Text = formateo((ObjReader1(6).ToString()), 1)
                oTable.Cell(conta_incerti, 8).Range.Text = Replace(FormatNumber(ObjReader1(6).ToString(), 2), ",", "")

                Dim lau = Val((ObjReader1(7).ToString()))
                oTable.Cell(conta_incerti, 9).Range.Text = lau
                oTable.Cell(conta_incerti, 10).Range.Text = lau.ToString("E1")
                'oTable.Rows.Item(conta_incerti).Height = oWord.CentimetersToPoints(0.3)
                oTable.Rows.Item(1).Height = 11
                conta_incerti = conta_incerti + 1
            End While
            ...
            For Each row As DataRow In datos_MasaConencional.Tables(0).Rows

                If i = 2 Then
                    oTable.Cell(i, 3).Range.Text = Replace(formateo(("0"), 1), ",", "")
                    i = i + 1
                    oTable.Cell(i, 3).Range.Text = Replace(formateo((row(0).ToString()), 1), ",", "")
                Else
                    oTable.Cell(i, 3).Range.Text = Replace(formateo((row(0).ToString()), 1), ",", "")
                End If
                i = i + 1
            ...
            Next
            oTable.Rows.Item(1).Range.Font.Bold = True
            oTable.Columns.Item(1).Width = oWord.CentimetersToPoints(0.9)
            oTable.Columns.Item(2).Width = oWord.CentimetersToPoints(2)
            oTable.Columns.Item(3).Width = oWord.CentimetersToPoints(2.8)
            oTable.Columns.Item(4).Width = oWord.CentimetersToPoints(2.7)
            oTable.Columns.Item(5).Width = oWord.CentimetersToPoints(2.6)
            oTable.Columns.Item(6).Width = oWord.CentimetersToPoints(2.5)
            oTable.Columns.Item(7).Width = oWord.CentimetersToPoints(1.6)
            oTable.Columns.Item(8).Width = oWord.CentimetersToPoints(1)
            oTable.Columns.Item(9).Width = oWord.CentimetersToPoints(1.5)
            oTable.Columns.Item(10).Width = oWord.CentimetersToPoints(1.5)
            oTable.Range.ParagraphFormat.Alignment = Word.WdParagraphAlignment.wdAlignParagraphCenter
    ...
    End Sub
    Private Sub Impresa_III(codigobpr As String, idebpr As String, u_recibida As String, tipo As String)
        ...
        Try
            ...
            If u_recibida.Equals("k") Then
                If tipo.Equals("P") Then

                    oDoc = oWord.Documents.Add("C:\archivos_metrologia\Plantillas\documentosQR\ClaseIII.dotx")
            ...
            Else
                ...
                oDoc = oWord.Documents.Add("C:\archivos_metrologia\Plantillas\documentosQR\ClaseIII_g.dotx")
            End If
            '///***
            Dim conteo As Integer = 0
            Dim Str2 As String = "select count(LitBpr) from Balxpro where idebpr=" & idebpr & "" ' modifcar 
            Dim ObjCmd2 As SqlCommand = New SqlCommand(Str2, ccn)
            Dim ObjReader2 = ObjCmd2.ExecuteReader
            While (ObjReader2.Read())
                conteo = Val(ObjReader2(0).ToString())
            ...
            'If tipo.Equals("P") Or u_recibida <> ("k") Then
            ...
            oDoc.Bookmarks.Item("telcli").Range.Text = telefono
            oDoc.Bookmarks.Item("recibecli").Range.Text = recibe
            oDoc.Bookmarks.Item("feccalibracion2").Range.Text = UCase(Format(Convert.ToDateTime(feccal), "yyyy-MM-dd"))
            oDoc.Bookmarks.Item("describal").Range.Text = instrumento
            oDoc.Bookmarks.Item("identbal").Range.Text = identificacion
            oDoc.Bookmarks.Item("marcabal").Range.Text = marca
            oDoc.Bookmarks.Item("modelobal").Range.Text = modelo
            ...
            oDoc.Bookmarks.Item("unidbal").Range.Text = unidad
            Dim cuenta_certif As Integer = 0
            Dim Str5 As String = "select distinct  count(REPLACE(replace(Cert_Balxpro.NomCer,'-UIO',''),'-GYE',''))  from Cert_Balxpro WHERE  (Cert_Balxpro.IdeComBpr = '" & IdeComBpr & "')"
            Dim ObjCmd5 As SqlCommand = New SqlCommand(Str5, ccn)
            Dim ObjReader5 = ObjCmd5.ExecuteReader
            While (ObjReader5.Read())
                cuenta_certif = Val(ObjReader5(0).ToString())
            ...
            fila = 2
            Dim certif As String, fec_cert As String, termoh As String
            Dim Str6 As String = "SELECT DISTINCT REPLACE(replace(NomCer,'-UIO',''),'-GYE','') " &
                                 "FROM     Cert_Balxpro WHERE  (Cert_Balxpro.IdeComBpr = '" & IdeComBpr & "')"
            Dim ObjCmd6 As SqlCommand = New SqlCommand(Str6, ccn)
            Dim ObjReader6 = ObjCmd6.ExecuteReader
            While (ObjReader6.Read())
                certif = ObjReader6(0).ToString()
                Certs = Certs & certif & ", "
                oTable.Cell(fila, 1).Range.Text = certif
                Dim Str7 As String = "select distinct(FecCer),TipCer from Certificados where REPLACE(replace(NomCer,'-UIO',''),'-GYE','') like '" & certif & "%' "
                Dim ObjCmd7 As SqlCommand = New SqlCommand(Str7, ccn)
                Dim ObjReader7 = ObjCmd7.ExecuteReader
                While (ObjReader7.Read())
                    fec_cert = ObjReader7(0).ToString()
            ...
            oTable.Rows.Item(1).Height = 12
            Str1 = "select CodPca_c,CarPca,NumPca from PCarga_Cab " &
                                     "where IdeComBpr = '" & IdeComBpr & "' ORDER BY convert(int,NUMPCA)"
            ObjCmd1 = New SqlCommand(Str1, ccn)
            ObjReader1 = ObjCmd1.ExecuteReader
            While (ObjReader1.Read())
                oTable.Cell(conta_lineas, 1).Range.Text = ObjReader1(2).ToString()
                ...
                Dim carga_r As String = Replace(formateo((ObjReader1(1).ToString()), 1), ",", "")

                Str2 = "Select LecAscPca,LecDscPca,LecAscPca - " & carga_r & ",LecDscPca - " & carga_r & ",EmpPca from Pcarga_Det " &
                                     "where CodPca_c = '" & IdeComBpr & ObjReader1(2).ToString() & "'" '" & (ObjReader1(0).ToString()) & ""
                ObjCmd2 = New SqlCommand(Str2, ccn)
                ObjReader2 = ObjCmd2.ExecuteReader
                While (ObjReader2.Read())
            ...
            '       "Proyectos ON Balxpro.CodPro = Proyectos.CodPro " & _
            '       "where Balxpro.IdeComBpr='" & IdeComBpr & "'"
            Str1 = "SELECT replace(fec_proxBpr,'-','/') FROM Balxpro  " &
                   "where IdeComBpr = '" & IdeComBpr & "'"
            ObjCmd1 = New SqlCommand(Str1, ccn)
            ObjReader1 = ObjCmd1.ExecuteReader
            While (ObjReader1.Read())
